...
const CarouselComponent = ({ title, params, Icon }: { title: string, params: any, Icon: React.ComponentType }) => {
  ...
  const isLeftButtonDisabled = scrollContainerRef.current?.scrollLeft === 0 && false;
  const isRightButtonDisabled = scrollContainerRef.current?.scrollLeft === (items?.length - 1) * (200+16);

  return (
    <div>
      <Header handleScrollLeft={handleScrollLeft} handleScrollRight={handleScrollRight} isLeftButtonDisabled={isLeftButtonDisabled} isRightButtonDisabled={isRightButtonDisabled} title={title} Icon={Icon} />
      <div ref={scrollContainerRef} className={`${styles.carouselContainer} ${styles.withShadow}`}>
        {useCasinosLoading ? (
